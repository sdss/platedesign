...
;-
;------------------------------------------------------------------------------
pro test_2mass_gsc, ra, dec, dra=dra, ddec=ddec

tilerad=0.5

objt = tmass_read(ra, dec, tilerad)

info= querygsc([ra, dec], tilerad*60.)
...
splot, 3600.*(objt[m2].tmass_ra-info[ii[m1]].ra)*cos(!DPI/180.*objt[m2].tmass_dec), $ 
...
dra= median(3600.*(objt[m2].tmass_ra-info[ii[m1]].ra)*cos(!DPI/180.*objt[m2].tmass_dec))
ddec= median(3600.*(objt[m2].tmass_dec-info[ii[m1]].dec))
