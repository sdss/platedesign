; This is a one-off plate made for the guide collar installation.
; See the similar plate in make_boss_alignment_test_plate
;
; This file generates an output file, 'plPlugMapP-8633x.par'. The proper header still
...
; to 'plPlugMapP-8633.par'.
;
; Once in its proper place: "$PLATELIST_DIR/plates/0086XX/008633/plPlugMapP-8633.par",
; the plate is finished by running:
;   IDL> platerun_bright, '2015.06.x.apogee', 8633, /nolines, /nocounterbores
;
; MRB, NYU 2015-06-16

pro make_apogee2s_alignment_plate
...
; units in mm
fiber_block_centers_y = [-205.74, -68.58, 22.86, 114.30, 205.74, -91.44, 0.00, 91.44, 205.74, 68.58, -22.86, -114.30, -205.74, 91.44, 0.00, -91.44]
fiber_block_centers_x = [-137.80, -137.80, -137.80, -137.80, -137.80, -277.50, -277.50, -277.50, 137.80, 137.80, 137.80, 137.80, 137.80, 277.50, 277.50, 277.50]
...
idx = where(fiber_block_centers_x ge 0)
index_hole_x[idx] = fiber_block_centers_x[idx] + (0.101 * 25.4)

idx = where(fiber_block_centers_x lt 0)
index_hole_x[idx] = fiber_block_centers_x[idx] - (0.101 * 25.4)

xyfocal2ad, 'APO', index_hole_y, index_hole_x, $
...
all_holes = [guide_holes, index_holes, center_hole]

dummy = yanny_readone(getenv('PLATEDESIGN_DIR') + '/data/sdss/plPlugMapP-blank.par', hdr=hdr, structs=structs)
yanny_write, 'plPlugMapP-8633x.par', ptr_new(all_holes), structs=structs, hdr=hdr
