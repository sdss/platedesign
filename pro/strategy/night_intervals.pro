...
; CALLING SEQUENCE:
;   intervals= night_intervals([date=, night=, duration= ])
; INPUTS:
...
;-
;------------------------------------------------------------------------------
function night_intervals, date=date, night=night, duration=duration, $
  overhead=overhead, minexp=minexp
...
end_lmst=(start_lmst+duration+overhead) MOD 24.
...
;; check if we can squeeze in an exposure; repeat commands 
;; from above here but adjust exposure time
end_lmst=(start_lmst+minexp+overhead) MOD 24.
if((night.lmst_morntwi gt night.lmst_eventwi AND $
    end_lmst lt night.lmst_morntwi AND $
    end_lmst gt night.lmst_eventwi) OR $
   (night.lmst_morntwi lt night.lmst_eventwi AND $
    (end_lmst lt night.lmst_morntwi OR $
     end_lmst gt night.lmst_eventwi))) then begin
    
    end_lmst= night.lmst_morntwi
    ...
    if(interval0.start_lmst gt interval0.end_lmst) then $
      interval0.exptime= interval0.end_lmst+(24.-interval0.start_lmst)- $
                         overhead $
    else $
      interval0.exptime= interval0.end_lmst-interval0.start_lmst-overhead
    interval0.moonup=moonup
    if(NOT moonup) then begin
        interval0.class='DARK'
    endif else begin
