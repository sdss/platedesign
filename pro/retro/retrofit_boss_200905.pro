...
for i=0L, n_elements(plans)-1L do begin
    ...
    if(nii gt 0) then begin
        ...
        sortedfile= platedir+'/plateHolesSorted-'+platestr6+'.par'
        plugmapfile= platedir+'/plPlugMapP-'+platestr+'.par'

        origholesfile= platedir+'/plateHoles-'+platestr6+'-orig.par'
        origsortedfile= platedir+'/plateHolesSorted-'+platestr6+'-orig.par'
        origplugmapfile= platedir+'/plPlugMapP-'+platestr+'-orig.par'

        if(file_test(origholesfile) eq 0) then begin
            spawn, 'cp '+holesfile+' '+origholesfile
            spawn, 'cp '+sortedfile+' '+origsortedfile
        ...
        plugfile_plplugmap_boss, plateid, /keepoldcoords
        spawn, 'cp -f '+plugmapfile+' '+getenv('PLATELIST_DIR')+'/runs/'+ $
               plateruns[ii[0]]
        
        platelines_boss, plateid, /sorty
    endif
endfor
