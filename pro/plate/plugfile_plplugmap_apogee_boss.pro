...
ihole= where(holes.holetype eq 'BOSS_SHARED', nhole)
if(nhole gt 0) then plug[ihole].holetype= 'OBJECT'
if(nhole gt 0) then plug[ihole].objtype= 'QSO'

ihole= where(holes.holetype eq 'BOSS_SHARED' and $
...
iapogee=where(strupcase(holes.holetype) eq 'APOGEE_SHARED', napogee)
...
iboss=where(strupcase(holes.holetype) eq 'BOSS_SHARED', nboss)
...
ihole= where(holes.holetype eq 'APOGEE_SHARED', nhole)
if(nhole gt 0) then begin
    isort= sort(abs(plug[ihole].fiberid))
    newplug=[newplug, plug[ihole[isort]]]
    newholes=[newholes, holes[ihole[isort]]]
