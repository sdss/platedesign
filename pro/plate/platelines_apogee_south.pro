...
;-
;------------------------------------------------------------------------------
pro platelines_apogee_south, in_plateid, diesoft=diesoft, sortx=sortx, relaxed=relaxed
  ...
  isci= where(strupcase(strtrim(full.holetype,2)) eq 'APOGEE_SOUTH', nsci)
     ...
     if(nsci gt 0) then begin
         ...
         for i=0L, nblocks-1L do begin
             ...
             if(nii gt 0) then begin
                 if(keyword_set(sortx)) then $
                   isort= sort(holes[isci[ii]].xfocal) $
                 else $
