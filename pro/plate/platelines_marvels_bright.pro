;+
; NAME:
;   platelines_marvels_bright
; PURPOSE:
;   write the plateLines-????.ps file for a MARVELS plate with 2011+ bright cartridges
; CALLING SEQUENCE:
;   platelines_marvels_bright, plateid 
; INPUTS:
...
;-
;------------------------------------------------------------------------------
pro platelines_marvels_bright, in_plateid
...
isci= where(strupcase(strtrim(full.holetype,2)) eq 'MARVELS', nsci)
...
linecolors=strarr(10,2)
linecolors[*,0]=['black', 'blue', 'green', 'red', 'yellow', $
                 'black', 'blue', 'green', 'red', 'yellow']
linecolors[*,1]= linecolors[*,0]
circlecolors= ['cyan','magenta','yellow','orange', 'green', 'purple']
...
for index=0L, npointings-1L do begin
    ...
    ;; select all fibers
    iobj= where(holes[isci].holetype eq 'OBJECT') ; index of OBJECTs
    ...
    for i=0L, nblocks-1L do begin
        ...
        ii= where(-holes[isci[iobj]].fiberid ge i*nper+1+foff and $  ; 1
                  -holes[isci[iobj]].fiberid le (i+1)*nper+foff, nii); 4
        ii=isci[iobj[ii]]
