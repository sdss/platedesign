...
if(gfiber[gnum-1].twist_axis eq 'y') then begin
...
endif else if (gfiber[gnum-1].twist_axis eq 'x') then begin
    if(gfiber[gnum-1].xreach gt 0.) then $
      thisang= (90.D) + twist_coeff*(xf_guide - gfiber[gnum-1].xreach) $
    else $
      thisang= -(90.D) - twist_coeff*(xf_guide - gfiber[gnum-1].xreach) 
    xf_align= xf_guide+align_hole_dist*sin(thisang/dradeg)
    yf_align= yf_guide-align_hole_dist*cos(thisang/dradeg)
endif 
